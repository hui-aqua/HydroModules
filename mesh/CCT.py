...
parameters = {
    ...
    'Frame':
    {
        ...
        # this value is better to be same with 'elementOverCir'.
        'elementOverRings': 36,
        'elementOverHeight': 8,
...
NN = int(cage_height/parameters['CageShape']['element_length'])
# number of section along the cone, thus, the nodes should be NN+1
BN = int(cr_bottom/parameters['CageShape']['element_length']/np.sqrt(2))
...
# the last point should be at the cone tip
point_one_cage.append([0, 0, -cage_cone_height-cage_height])
    ...
    for j in range(0, NN_frame + 1):
        point_one_cage.append(
            [(r_top-dr*j/float(NN_frame)) * np.cos(i * 2 * pi / float(NT_frame)),
             (r_top-dr*j/float(NN_frame)) *
             np.sin(i * 2 * pi / float(NT_frame)),
...
for center in floater_center:
    point += (np.array(point_one_cage) +
              np.ones((len(point_one_cage), 3))*np.array(center)).tolist()
...
for j in range(NT_frame):
    ...
    con_in_one_frame_top.append(
        [1+j*(BN+NN), 1+j*(NN_frame+1)+number_of_point_one_netting])
    # circular
    if j < NT_frame-1:
        con_in_one_frame_top.append(
            [1+j*(NN_frame+1)+number_of_point_one_netting, 1+(j+1)*(NN_frame+1)+number_of_point_one_netting])
        con_in_one_frame_bottom.append(
            [1+NN_frame+j*(NN_frame+1)+number_of_point_one_netting, 1+NN_frame+(j+1)*(NN_frame+1)+number_of_point_one_netting])
    else:
        con_in_one_frame_top.append(
            [1+j*(NN_frame+1)+number_of_point_one_netting, 1+number_of_point_one_netting])
        con_in_one_frame_bottom.append(
            [1+NN_frame+j*(NN_frame+1)+number_of_point_one_netting, 1+NN_frame+number_of_point_one_netting])
    # vertical lines
    for i in range(NN_frame):
        con_in_one_frame_line.append(
            [1+i+j*(NN_frame+1)+number_of_point_one_netting, 1+1+i+j*(NN_frame+1)+number_of_point_one_netting])
    # bottom
    con_in_one_frame_line.append(
        [1+NN+j*(BN+NN), 1+NN_frame+j*(NN_frame+1)+number_of_point_one_netting])


# Step 2
sur_netting_tri = []
con_one_cage = con_in_one_netting+con_in_one_frame_line + \
    con_in_one_frame_top+con_in_one_frame_bottom
...
for num in range(len(floater_center)):
    sur_netting += (np.array(sur_in_one_netting) + num *
                    number_of_point_one_cage).tolist()
    sur_netting_tri += (np.array(sur_in_one_netting_tri) +
                        num*number_of_point_one_cage).tolist()
    con_all += (np.array(con_one_cage) + num*number_of_point_one_cage).tolist()
    con_pipe_top += (np.array(con_in_one_frame_top) +
                     num*number_of_point_one_cage).tolist()
    con_pipe_bottom += (np.array(con_in_one_frame_bottom) +
                        num*number_of_point_one_cage).tolist()
    con_rope += (np.array(con_in_one_frame_line) +
                 num*number_of_point_one_cage).tolist()
    con_netting += (np.array(con_in_one_netting) + num *
                    number_of_point_one_cage).tolist()
...
for num in range(len(floater_center)):
    ...
    nbAdd = net_top.Add([i for i in range(
        index_start+1, index_start+number_of_point_one_netting, BN+NN)])
...
for num in range(len(floater_center)):
    ...
    nbAdd = net_bottom.Add([i for i in range(
        index_start+1, index_start+number_of_point_one_netting, BN+NN)])
...
for num in range(len(floater_center)):
    ...
    nbAdd = ring_top.Add([i for i in range(
        index_start+1, index_start+number_of_point_one_frame+1, NN_frame+1)])
...
for num in range(len(floater_center)):
    ...
    nbAdd = ring_bottom.Add([i for i in range(
        index_start+1, index_start+number_of_point_one_frame+1, NN_frame+1)])
...
for num in range(len(floater_center)+1):
    nbAdd = bottom_tip.Add(
        [number_of_point_one_netting+number_of_point_one_cage*num])
...
for num in range(len(floater_center)):
    ...
    nbAdd = twines.Add(
        [i for i in range(index_start+1+NT, index_start+1+len(con_in_one_netting))])
...
for num in range(len(floater_center)):
    ...
    nbAdd = ropes.Add(
        [i for i in range(index_start+1, index_start+1+len(con_in_one_frame_line))])
...
for num in range(len(floater_center)):
    index_start = len(con_in_one_netting) + \
        len(con_in_one_frame_line)+num_of_con_in_one_cage*num
    nbAdd = topRings.Add(
        [i for i in range(index_start+1, index_start+1+len(con_in_one_frame_top))])
    nbAdd = topRings.Add([i for i in range(
        num*num_of_con_in_one_cage+1, num*num_of_con_in_one_cage+1+NT)])
smesh.SetName(topRings, 'topRings')

# # defaults names for bottom ring
bottomRings = Mesh_1.CreateEmptyGroup(SMESH.EDGE, 'bottomRings')
for num in range(len(floater_center)):
    index_start = len(con_in_one_netting)+len(con_in_one_frame_line) + \
        len(con_in_one_frame_top)+num_of_con_in_one_cage*num
    nbAdd = bottomRings.Add(
        [i for i in range(index_start+1, index_start+1+len(con_in_one_frame_bottom))])
...
fb_netting = total_volume_of_twine*9.81 * \
    parameters['Environment']['fluidDensity']/number_of_point_one_netting
...
print('Mesh file is generated and stored at ' +
      str(os.path.join(os.getcwd(), meshname)))
